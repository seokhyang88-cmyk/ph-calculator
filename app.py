...
st.set_page_config(page_title="공학용 pH 마스터 v4.5", layout="centered")
...
# 각 메뉴별 입력창 세팅 (숫자 값을 연동하기 위해 세션스테이트 기반으로 바인딩)
if current_mode == "1":
    col1, col2 = st.columns(2)
    with col1: st.session_state.p_val = st.number_input("농도 앞자리 (예: 5.8):", value=float(st.session_state.p_val), step=0.1, key="m1_p")
    with col2: st.session_state.e_val = st.number_input("10의 마이너스 지수 (예: -4):", value=int(st.session_state.e_val), step=1, key="m1_e")

elif current_mode == "2":
    col1, col2 = st.columns(2)
    with col1: st.session_state.p_val = st.number_input("농도 앞자리 (예: 4.0):", value=float(st.session_state.p_val), step=0.1, key="m2_p")
    with col2: st.session_state.e_val = st.number_input("10의 마이너스 지수 (예: -2):", value=int(st.session_state.e_val), step=1, key="m2_e")

# ... (나머지 3번~10번 입력 인터페이스는 깔끔한 연동을 위해 이전과 동일하게 유지됩니다)
elif current_mode == "3":
...
# --- ⌨️ 3. 이미지 대신 직접 버튼들로 배치한 [물리 키패드 패널] ---
st.write("⌨️ PHYSICAL KEYPAD FRAME")

# 공학용 계산기 특수 기호 패드 라인
row1_1, row1_2, row1_3, row1_4 = st.columns(4)
with row1_1: st.button("SHIFT", key="k_shift", use_container_width=True)
with row1_2: st.button("ALPHA", key="k_alpha", use_container_width=True)
with row1_3: st.button("MODE", key="k_mode", use_container_width=True)
with row1_4: 
    if st.button("AC", key="k_ac", type="secondary", use_container_width=True):
        st.session_state.ph_result = "SYSTEM RESET 완료"
        st.clear_cache()

row2_1, row2_2, row2_3, row2_4 = st.columns(4)
with row2_1: st.button("▶", key="k_r", use_container_width=True)
with row2_2: st.button("x²", key="k_x2", use_container_width=True)
with row2_3: st.button("xʸ", key="k_xy", use_container_width=True)
with row2_4: st.button("√ ", key="k_root", use_container_width=True)

row3_1, row3_2, row3_3, row3_4 = st.columns(4)
with row3_1: st.button("log", key="k_log", use_container_width=True)
with row3_2: st.button("ln", key="k_ln", use_container_width=True)
with row3_3: st.button("sin", key="k_sin", use_container_width=True)
with row3_4: st.button("cos", key="k_cos", use_container_width=True)

st.write("")

# 일반 숫자 패드 라인
num1, num2, num3, op1 = st.columns(4)
with num1: st.button("7", key="k_7", use_container_width=True)
with num2: st.button("8", key="k_8", use_container_width=True)
with num3: st.button("9", key="k_9", use_container_width=True)
with op1:  st.button("DEL", key="k_del", use_container_width=True)

num4, num5, num6, op2 = st.columns(4)
with num4: st.button("4", key="k_4", use_container_width=True)
with num5: st.button("5", key="k_5", use_container_width=True)
with num6: st.button("6", key="k_6", use_container_width=True)
with op2:  st.button("×", key="k_mul", use_container_width=True)

num7, num8, num9, op3 = st.columns(4)
with num7: st.button("1", key="k_1", use_container_width=True)
with num8: st.button("2", key="k_2", use_container_width=True)
with num9: st.button("3", key="k_3", use_container_width=True)
with op3:  st.button("＋", key="k_add", use_container_width=True)

# 가장 밑줄의 핵심 연산버튼 (Exp, ＝)
num0, num_dot, num_exp, op_equal = st.columns(4)
with num0: st.button("0", key="k_0", use_container_width=True)
with num_dot: st.button(".", key="k_dot", use_container_width=True)
with num_exp: st.button("Exp", key="k_exp", use_container_width=True)

with op_equal:
    # 이 버튼을 누르면 위의 수식 조건에 맞춰 실시간으로 상단 모니터에 정답이 꽂힙니다.
    if st.button(" ＝ ", type="primary", key="k_equal", use_container_width=True):
        try:
