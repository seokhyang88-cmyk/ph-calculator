...
# 스마트폰 화면에 맞게 페이지 설정
st.set_page_config(page_title="화학 pH 마스터 v2.2", layout="centered")

# 눈에 너무 띄던 상단 큰 제목과 문구를 없애고, 균일한 크기의 텍스트로 시작합니다.
...
if menu == "1. H+ 이온 농도로 pH 구하기":
    ...
    if st.button("정답 확인", type="primary", key="u1_btn"):
        if hp_prefix <= 0:
            ...
            st.write(f"최종 계산 정답: pH = {final_ph_val:.2f}")
    ...
    if st.button("정답 확인", type="primary", key="u3_btn"):
        ...
        if use_log_hint:
            ...
            if abs(diff - log_value) < 0.02:
                st.write(f"시험 보기 정답 형태: {log_base_num} * 10^-{floor_val} mol/L")
...
# --- 유형 4: 같은 성질 혼합 (N농도) ---
    ...
    if st.button("정답 확인", type="primary", key="u4_btn"):
        ...
        if sol_kind == "산성 용액끼리 섞음":
            st.write(f"최종 혼합 용액의 정답 pH = {-math.log10(final_n):.2f}")
        else:
            st.write(f"최종 혼합 용액의 정답 pH = {14.0 - (-math.log10(final_n)):.2f}")
...
# --- 유형 5: 강산/강염기 단일 pH ---
    ...
    if st.button("정답 확인", type="primary", key="u5_btn"):
        ...
        if "강산성" in sol_type:
            st.write(f"최종 pH = {-math.log10(final_n):.2f}")
        else:
...
# --- 유형 7: 중화 적정 실험 전용 ---
    ...
    if st.button("정답 확인", type="primary", key="u7_btn"):
        ...
        if "강산" in target_type: ph = -math.log10(n_result)
...
# --- 유형 9: 농도 단위 교차 환산기 ---
    ...
    with col_i1: mw = st.number_input("물질의 분자량:", value=40.0, step=1.0, key="u9_mw")
    with col_i2: val = st.number_input("물질의 가수:", value=1, step=1, key="u9_val")
    ...
    if "약전 표기법" in input_mode:
        ...
        wv_percent = st.number_input("W/V % 값:", value=1.0, format="%.2f", step=0.1, key="u9_wv")
