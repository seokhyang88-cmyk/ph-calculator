...
st.write("농도 계산과 단위 환산 기출문제를 모두 풀 수 있는 종합 도우미 앱입니다.")
...
menu = st.selectbox(
    "풀고 싶은 문제 유형을 선택하세요:",
    [
        "1. 🧪 [신규] H+ 이온 농도로 OH- 이온 농도 구하기",
        "2. pH 값을 주고 수소 이온 농도 [H+] 역산하기",
        "3. 같은 성질의 산성/염기성 용액 혼합하기 (N 농도 기반)",
        "4. 강산 또는 강염기 단일 용액의 pH 구하기",
        "5. 두 용액을 섞은 혼합 용액의 pH 구하기 (산+염기 또는 같은 액성 혼합)",
        "6. 중화 적정 실험 데이터로 미지 용액의 농도 및 pH 구하기",
        "7. 약산/약염기 pH 계산기 (전리도 또는 해리상수 Ka/Kb 선택)",
        "8. 📊 농도 단위 교차 환산기 (M, N, ppm, % 무조건 변환)",
        "9. 수산화 이온 농도 [OH-]로 곧바로 pH 구하기"
    ]
)

st.markdown("---")

# --- 🌟 [신규 탑재] 유형 1: H+ 농도로 OH- 농도 구하기 🌟 ---
if menu == "1. 🧪 [신규] H+ 이온 농도로 OH- 이온 농도 구하기":
    st.subheader("🧪 수소 이온 농도 기반 수산화 이온 농도 계산기")
    st.write("H+ 이온 농도를 지수 형태로 입력하면 물의 이온화 상수를 이용해 OH- 농도를 도출합니다.")
    st.write("예: H+ 이온의 농도가 4.0 X 10^-2 mole/L 일 때 OH- 이온의 농도는?")
    
    col_h1, col_h2 = st.columns(2)
    with col_h1: h_prefix = st.number_input("농도 앞자리 숫자 (예: 4.0)", value=4.0, step=0.1, key="u1_hp")
    with col_h2: h_exponent = st.number_input("10의 마이너스 지수 (예: -2)", value=-2, step=1, key="u1_he")
    ...
    if st.button("🎉 OH- 이온 농도 정답 확인", type="primary"):
        if h_prefix <= 0:
            ...
            # 물의 이온화 상수 Kw = 1.0 * 10^-14를 이용해 OH- 농도 역산
            oh_conc = 1e-14 / h_conc
            
            # 결과를 보기 좋게 지수 문자로 분리 표기하기 위한 로직
            oh_str = f"{oh_conc:.2e}"
            ...
            st.success(f"🎉 **최종 정답: OH⁻ 이온 농도 = {base} * 10^{exponent_val} mole/L**")

# --- 유형 2 (기존 1번) ---
elif menu == "2. pH 값을 주고 수소 이온 농도 [H+] 역산하기":
...
# --- 유형 3 (기존 2번) ---
elif menu == "3. 같은 성질의 산성/염기성 용액 혼합하기 (N 농도 기반)":
...
# --- 유형 4 (기존 3번) ---
elif menu == "4. 강산 또는 강염기 단일 용액의 pH 구하기":
...
# --- 유형 5 (기존 4번) ---
elif menu == "5. 두 용액을 섞은 혼합 용액의 pH 구하기 (산+염기 또는 같은 액성 혼합)":
...
# --- 유형 6 (기존 5번) ---
elif menu == "6. 중화 적정 실험 데이터로 미지 용액의 농도 및 pH 구하기":
...
# --- 유형 7 (기존 6번) ---
elif menu == "7. 약산/약염기 pH 계산기 (전리도 또는 해리상수 Ka/Kb 선택)":
...
# --- 유형 8 (기존 7번) ---
elif menu == "8. 📊 농도 단위 교차 환산기 (M, N, ppm, % 무조건 변환)":
    st.subheader("📊 상호 교차 농도 변환기")
    st.write("질량 정보 없이 분자량과 가수를 기준으로 모든 농도 수치를 자유롭게 교차 변환합니다.")
    col_i1, col_i2 = st.columns(2)
    with col_i1: mw = st.number_input("물질의 분자량 (예: NaOH=40, HCl=36.5)", value=40.0, step=1.0, key="u7_mw_fixed")
    with col_i2: val = st.number_input("물질의 가수 (H나 OH 개수)", value=1, step=1, key="u7_val_fixed")
    st.markdown("---")
    input_mode = st.radio(
        "현재 내가 알고 있는 힌트 농도는 무엇인가요?",
        ["약전 표기법 (예: 1 → 100 방식)", "W/V % 농도 값 직접 입력", "몰 농도 (M) 값 직접 입력", "노르말 농도 (N) 값 직접 입력", "ppm 농도 값 직접 입력"],
        key="u7_input_mode_radio"
    )
    wv_percent = 0.0
    if input_mode == "약전 표기법 (예: 1 → 100 방식)":
        col_solute, col_solvol = st.columns(2)
        with col_solute: g_val = st.number_input("녹인 고체 양 (g)", value=1.0, step=0.1, key="u7_g")
        with col_solvol: ml_val = st.number_input("전체 용액 부피 (mL)", value=100.0, step=10.0, key="u7_ml")
        if ml_val > 0: wv_percent = (g_val / ml_val) * 100.0
    elif input_mode == "W/V % 농도 값 직접 입력":
        wv_percent = st.number_input("W/V % 값을 입력하세요:", value=1.0, format="%.2f", step=0.1, key="u7_wv")
    elif input_mode == "몰 농도 (M) 값 직접 입력":
        m_val = st.number_input("몰 농도 (M) 값을 입력하세요:", value=0.25, format="%.4f", step=0.01, key="u7_m")
        wv_percent = (m_val * mw) / 10.0
    elif input_mode == "노르말 농도 (N) 값 직접 입력":
        n_val = st.number_input("노르말 농도 (N) 값을 입력하세요:", value=0.25, format="%.4f", step=0.01, key="u7_n")
        if val > 0: wv_percent = ((n_val / val) * mw) / 10.0
    elif input_mode == "ppm 농도 값 직접 입력":
        ppm_val = st.number_input("ppm 값을 입력하세요:", value=10000.0, format="%.1f", step=100.0, key="u7_ppm")
        wv_percent = ppm_val / 10000.0
    st.markdown("---")
    if st.button("🔄 나머지 모든 농도로 일괄 변환하기", type="primary", key="u7_calc_btn"):
...
# --- 유형 9 (기존 8번) ---
elif menu == "9. 수산화 이온 농도 [OH-]로 곧바로 pH 구하기":
