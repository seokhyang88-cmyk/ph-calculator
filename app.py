...
# --- 🌟 [오류 수정 및 구조 재안정화] 유형 7: 농도 단위 교차 환산기 🌟 ---
elif menu == "7. 📊 농도 단위 교차 환산기 (M, N, ppm, % 무조건 변환)":
    st.subheader("📊 상호 교차 농도 변환기")
    st.write("질량 정보 없이 분자량과 가수를 기준으로 모든 농도 수치를 자유롭게 교차 변환합니다.")
    
    # 레이아웃 간섭을 피하기 위해 key 값을 고유하게 설정
    ...
    # 모바일 렌더링 에러를 막기 위한 명확한 라디오 버튼 정의
    input_mode = st.radio(
