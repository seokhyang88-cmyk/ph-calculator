...
menu = st.selectbox(
    ...
    [
        ...
        "7. 🧪 [신규] 중화 적정 실험 데이터로 미지 용액의 pH 구하기"
    ]
)
...
# --- 유형 6 ---
    ...
    st.subheader("🧪 약산/약염기 pH 종합 계산기")
    acid_base_type = st.radio("용액의 성질을 고르세요:", ["약산성 용액 (CH3COOH 등)", "약염기성 용액 (NH4OH 등)"])
    given_type = st.radio("문제에서 주어진 힌트 유형은 무엇인가요?", ["해리 상수 (Ka 또는 Kb)", "전리도 (alpha, 해리도)"])
    st.markdown("---")
    molarity = st.number_input("용액의 몰 농도 (M)", value=0.100, format="%.4f", step=0.001)
    if given_type == "해리 상수 (Ka 또는 Kb)":
        col_k1, col_k2 = st.columns(2)
        with col_k1: k_prefix = st.number_input("상수 앞 숫자 (예: 1.8)", value=1.8, step=0.1, key="kp")
        with col_k2: k_exponent = st.number_input("10의 지수 (예: -5)", value=-5, step=1, key="ke")
        if st.button("🎉 정답 확인", type="primary", key="b6_k"):
            ...
            if acid_base_type == "약산성 용액 (CH3COOH 등)":
                ...
                st.info(f"🧬 **수소 이온 농도 [H⁺]** = √(C × Ka) = **{ion_conc:.6f} mol/L**")
                st.success(f"🎉 **최종 정답 pH = {ph:.2f}**")
            else:
                poh = -math.log10(ion_conc)
                ph = 14.0 - poh
                st.info(f"🧬 **수산화 이온 농도 [OH⁻]** = √(C × Kb) = **{ion_conc:.6f} mol/L**")
                st.info(f"🧪 **pOH** = -log10({ion_conc:.6f}) = **{poh:.2f}**")
                st.success(f"🎉 **최종 정답 pH (14 - pOH) = {ph:.2f}**")
    else:
        ionization_input = st.number_input("전리도(해리도) 입력 (값 또는 %)", value=1.0, format="%.5f", step=0.1)
        input_type = st.radio("전리도 입력 단위를 선택하세요:", ["퍼센트 단위 (%)", "소수점 단위 (0~1 사이의 값)"])
        if st.button("🎉 정답 확인", type="primary", key="b6_a"):
            ...
            if alpha <= 0 or alpha > 1: st.error("❌ 전리도 값이 올바르지 않습니다.")
                ...
                if acid_base_type == "약산성 용액 (CH3COOH 등)":
                    ph = -math.log10(ion_conc)
                    st.info(f"🧬 **수소 이온 농도 [H⁺]** = C × alpha = **{ion_conc:.5f} mol/L**")
                    st.success(f"🎉 **최종 정답 pH = {ph:.2f}**")
                else:
                    poh = -math.log10(ion_conc)
                    ph = 14.0 - poh
                    st.info(f"🧬 **수산화 이온 농도 [OH⁻]** = C × alpha = **{ion_conc:.5f} mol/L**")
                    st.info(f"🧪 **pOH** = -log10({ion_conc:.5f}) = **{poh:.2f}**")
                    st.success(f"🎉 **최종 정답 pH (14 - pOH) = {ph:.2f}**")

# --- 🌟 [신규] 유형 7: 중화 적정 실험 계산기 🌟 ---
elif menu == "7. 🧪 [신규] 중화 적정 실험 데이터로 미지 용액의 pH 구하기":
    st.subheader("🧪 중화 적정 미지 농도 및 pH 도출")
    st.write("실험을 통해 소비된 적정 용액의 부피를 이용하여 모르는 용액의 pH를 구합니다.")
    
    target_type = st.radio("농도를 알아내려는 미지의 용액 종류는?", ["농도를 모르는 강산 (HCl 등)", "농도를 모르는 강염기 (NaOH 등)"])
    
    st.markdown("### 🧪 1. 실험에 사용한 미지 용액의 조건")
    v_target = st.number_input("미지 용액의 취한 부피 (mL)", value=100, step=10, key="u7_vt")
    
    st.markdown("### 🧪 2. 뷰렛에 넣고 떨어뜨린 표준 용액(농도를 아는 용액)의 조건")
    n_standard = st.number_input("표준 용액의 노르말 농도 (N)", value=0.1000, format="%.4f", step=0.01, key="u7_ns")
    v_standard = st.number_input("적정에 소비된 표준 용액의 부피 (mL)", value=20, step=1, key="u7_vs")
    
    st.markdown("---")
    
    if st.button("🎉 중화 적정 정답 확인", type="primary"):
        if v_target <= 0 or n_standard <= 0 or v_standard <= 0:
            st.error("❌ 모든 입력값은 0보다 커야 합니다.")
        else:
            # N * V = N' * V' 공식을 이용한 미지 용액의 노르말 농도(N_result) 유도
            n_result = (n_standard * v_standard) / v_target
            
            st.markdown("### 📋 중화 적정 분석 결과")
            st.info(f"📊 **계산된 미지 용액의 농도** = ({n_standard} N × {v_standard} mL) ÷ {v_target} mL = **{n_result:.4f} N**")
            
            if target_type == "농도를 모르는 강산 (HCl 등)":
                # 강산이므로 노르말 농도가 곧 [H+] 농도
                ph = -math.log10(n_result)
                st.success(f"🎉 **미지 강산 용액의 최종 pH = {ph:.2f}**")
                st.warning(f"💡 정수 형태의 보기를 고르는 시험 문제라면 가장 가까운 **{round(ph)}**를 선택하세요!")
            else:
                # 강염기이므로 노르말 농도가 곧 [OH-] 농도
                poh = -math.log10(n_result)
                ph = 14.0 - poh
                st.success(f"🎉 **미지 강염기 용액의 최종 pH = {ph:.2f}**")
                st.warning(f"💡 정수 형태의 보기를 고르는 시험 문제라면 가장 가까운 **{round(ph)}**를 선택하세요!")